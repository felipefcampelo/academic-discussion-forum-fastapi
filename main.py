...
@app.get("/api/index")
def index():
    return {"message": "Welcome to Academic Discussion Forum API"}
...
@app.get("/api/list/users/pending")
def list_pending_users():
    return pending_users

@app.delete("/api/delete/users/pending")
def delete_pending_users(accounts: List[str] = []):
    for user in accounts:
        del pending_users[user]
    return {"message": "deleted pending users"}

@app.post("/api/login/validate", response_model=ValidUser)
...
@app.get("/api/login/details/info")
def login_info():
    return {"message": "username and password are needed"}

@app.get("/api/login/password/change")
...
@app.post("/api/login/username/unlock")
...
@app.post("/api/login/password/unlock")
...
@app.post("/api/account/profile/add", response_model=UserProfile)
def add_profile(username: str, 
    ...
    if valid_users.get(username) == None:
        return UserProfile(firstname=None, lastname=None, middle_initial=None, 
                           age=None, birthday=None, salary=None, user_type=None)
    else:
...
@app.put("/api/account/profile/update/{username}")
...
@app.patch("/api/account/profile/update/names/{username}")
...
@app.get("/api/account/profile/view/{username}")
...
@app.post("/api/discussion/posts/add/{username}")
def post_discussion(username: str, post: Post, post_type: PostType):
    if valid_users.get(username) == None:
        ...
        forum_post = ForumPost(id=uuid1(), topic=post.topic, message=post.message,
                               post_type=post_type, date_posted=post.date_posted,
                               username=username)
...
@app.post("/api/discussion/posts/reply/{username}")
...
@app.put("/api/discussion/posts/update/{username}")
...
@app.delete("/api/discussion/posts/remove/{username}")
...
@app.get("/api/discussion/posts/view/{username}")
...
@app.post("/api/login/rememberme/create/")
